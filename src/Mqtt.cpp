...
void Mqtt::_reconnect()
{
    if (!client.connected())
    {
        ...
        else
        {
            ...
            this->sendHADiscoveryMsg(doc, "last15s");
            doc.clear();
            ...
            this->sendHADiscoveryMsg(doc, "last60s");
            doc.clear();

            doc["name"] = "Geiger Counter µSv/h";
            doc["stat_t"] = TOPIC_USV;
            doc["unit_of_meas"] = "µSv/h";
            doc["frc_upd"] = true;
            doc["icon"] = "mdi:atom-variant";
            this->sendHADiscoveryMsg(doc, "uSv");
...
void Mqtt::sendHADiscoveryMsg(DynamicJsonDocument doc, const char *objectName)
{
    String objectId = String(this->config.clientId);
